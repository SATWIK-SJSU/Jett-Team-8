...
export const Ticketbookingform = () => {
  ...
  var premium = location.state.premium;
  ...
  const paymentFunction = e => {
    ...
    var ticketrate = ticketcost * ceats;
    var currentday = currentDate.getDay();
// alert("Ticket rate with no of seats : "+ticketrate);
    e.preventDefault();
    
    if (offer != "" && offer > 0 && currentday == 2) {
      alert("offer is : "+offer)
      var offerpercent = ticketrate * (offer / 100);
      var ticketrate = ticketrate - offerpercent;
    // alert("ticket rate after offer : "+ticketrate)
    }
    if (premium == "N") {
      ticketrate = ticketrate + 1.5;
      // alert("ticket rate after service charge : "+ticketrate)
    }
    var options = {
      key: "rzp_test_VCHkiYsUqptw9b",
      key_secret: "NE7dd4fZU9QKPgw2dZ2PEUsO",
  ...
  return (
    <div class="form-container sign-in-container" className="ticket-booking">
      <form
        className="ceat-pick"
        style={{ background: "#f6f5f7", marginTop: "20%" }}
      >
